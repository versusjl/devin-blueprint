...
> **not** restate the lanes, review gates, or Linear sync rules — it only
> describes how **Codex** runs the coordinator role. The shared rules win on
> any conflict.
...
[`worker-prompts.md`](./worker-prompts.md); the lanes, review gates, and Linear sync are
the shared ones from `loops.md` and `labels.md`.
...
review (four gates: review bots → opposite-agent peer review → Devin Sr. review → human
merge). The unattended work loop lives in [loops.md](loops.md).
...
- Give each worker the self-contained prompt from guides/worker-prompts.md, filled
...
- Run the project's type-check. For user-visible/terminal behavior, run the manual smoke
  checks.
...
- Check correctness, acceptance criteria, missing tests, edge cases, regressions,
  maintainability, over-broad scope, and the repo's non-negotiables (AGENTS.md).
...
- PR body MUST contain "Closes <TEAM>-<n>" so Linear links and auto-moves on merge.
...
- Note: branch protection requires CODEOWNERS / human approval, so an unattended thread
  cannot self-merge. Authorized merge means an attended run where the human operator has
  cleared it.
...
- Acceptance criteria unclear; tracker/push/PR/merge permissions fail; a design decision
  ...
  would violate one of the repo's non-negotiables (AGENTS.md).
...
- Issue ID and title; Linear status; branch; worktree path (if used); PR URL; merge result;
  ...
  (guides/child-session-handoff.md).
...
unattended work loop is the scheduled tick in [loops.md](loops.md).
